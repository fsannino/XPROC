...
function goMenus()
{
	...
	********************************************************************************/	
	...
	oMenu.mainbgcoloroff='#ADADAD' //#0066FF - Background do menu principal
	oMenu.mainbgcoloron='#6C6C6C' //0099FF - Background do mouseover do menu principal
	oMenu.subbgcoloroff='#003399' //Background do submenu
	oMenu.subbgcoloron='#6C6C6C' // #0066CC - Background do mouseover do submenu
	oMenu.subsubbgcoloroff='#003399' //#003399 - Background do subsubmenu
	oMenu.subsubbgcoloron='#6C6C6C' //#0066CC - Background do mouseover do subsubmenu
	...
	{
		case 'indexA.js':	
			oMenu.menuplacement=new Array('0','100','200','300','400'); //os primeiros 3 números são a posição horizontal dos menus
			break;
			
		case 'indexB.js':
			oMenu.menuplacement=new Array('0','100','200','300','400'); //*** Posição horizontal dos menus
			break;
		...
		case 'indexD.js':	
			oMenu.menuplacement=new Array('0','70'); //*** Posição horizontal dos menus
			break;
	...
	********************************************************************************/
	...
	{
		case 'indexA.js':				
			cadastros(0); //*** REFERENTE AO CADASTRO	
			corte(1); //*** REFERENTE AO CORTE
			usuario(2); //*** REFERENTE A USUÁRIO	
			feriado(3); //*** REFERENTE A FERIADO
			consulta(4); //*** REFERENTE AO CONSULTA			
			break;
		
		case 'indexB.js':		
			cadastros(0); //*** REFERENTE AO CADASTRO				
			consulta(1); //*** REFERENTE A CONSULTA				
			break;
			
		case 'indexC.js':		
			cadastros(0); //*** REFERENTE AO CADASTRO	
			corte(1); //*** REFERENTE AO CORTE
			feriado(2); //*** REFERENTE A FERIADO
			consulta(3); //*** REFERENTE A CONSULTA		
			break;
		...
		case 'indexQ.js':			
			cadastros(0); //*** REFERENTE AO CADASTRO	
			corte(1); //*** REFERENTE AO CORTE
			usuario(2); //*** REFERENTE A CONSULTA
			feriado(3); //*** REFERENTE A CONSULTA
			consulta(4); //*** REFERENTE A CONSULTA		
			troca_server(5); //*** REFERENTE AO TROVA DE SERVIDOR	
			break;
...
function cadastros(posicao)
{	
	oMenu.makeMain(posicao,'CADASTRO',0)	
	
	qtd_1=8;			
	oMenu.makeSub(posicao,0,'Curso',raiz + '/asp/grade/sel_curso.asp',qtd_1)	
	oMenu.makeSub(posicao,1,'Centro de Treinamento',raiz + '/asp/grade/sel_CT.asp',qtd_1)	
	oMenu.makeSub(posicao,2,'Unidade',raiz + '/asp/grade/sel_unidade.asp',qtd_1)	
	oMenu.makeSub(posicao,3,'Localidade',raiz + '/asp/grade/sel_localidade.asp',qtd_1)	
	oMenu.makeSub(posicao,4,'Sala',raiz + '/asp/grade/sel_sala.asp',qtd_1)
	oMenu.makeSub(posicao,5,'Multiplicador',raiz + '/asp/grade/sel_multiplicador.asp',qtd_1)
	oMenu.makeSub(posicao,6,'Multiplic. Aprovados',raiz + '/asp/grade/sel_multiplicador_aprov.asp',qtd_1)
	oMenu.makeSub(posicao,7,'Turma',raiz + '/asp/grade/inclui_altera_turma.asp',qtd_1)	
}

function corte(posicao)
{
	oMenu.makeMain(posicao,'CORTE',0)	
	qtd_1=1;	
	oMenu.makeSub(posicao,0,'Cadastro',raiz + '/asp/grade/sel_corte.asp',qtd_1)
}
...
function usuario(posicao)
{
	...
	qtd_1=1;	
	oMenu.makeSub(posicao,0,'Cadastro',raiz + '/asp/grade/cadastra_usuario.asp',qtd_1)
}

function feriado(posicao)
{
	oMenu.makeMain(posicao,'FERIADO',0)	
	qtd_1=1;	
	oMenu.makeSub(posicao,0,'Cadastro',raiz + '/asp/grade/sel_feriado.asp',qtd_1)
}

function troca_server(posicao)
{
	oMenu.makeMain(posicao,'SUPORTE',0)	
	qtd_1=1;	
	oMenu.makeSub(posicao,0,'Troca de Servidor',raiz + '/asp/grade/troca_servidor.asp',qtd_1)
}
...
function consulta(posicao)
{
    ...
    qtd_6 = 13;
	oMenu.makeSub(posicao,0,'Diretoria',raiz + '/asp/grade/dados_diretoria.asp?pTituloRel=Diretorias',qtd_6)
	oMenu.makeSub(posicao,1,'Unidade',0,qtd_6)	
	oMenu.makeSub(posicao,2,'Localidade',raiz + '/asp/grade/dados_localidade.asp?pTituloRel=Localidade',qtd_6)
	oMenu.makeSub(posicao,3,'Centro Treinamento',raiz + '/asp/grade/dados_centro_treinamento.asp?pTituloRel=Centro Treinamento',qtd_6)
	oMenu.makeSub(posicao,4,'Sala',raiz + '/asp/grade/dados_sala.asp?pTituloRel=Sala',qtd_6)
	oMenu.makeSub(posicao,5,'Feriado',raiz + '/asp/grade/dados_feriado.asp?pTituloRel=Feriado',qtd_6)
	oMenu.makeSub(posicao,6,'Feriado x Sala',raiz + '/asp/grade/dados_feriado_sala.asp?pTituloRel=Feriado x Sala',qtd_6)
	oMenu.makeSub(posicao,7,'Orgão',0,qtd_6)
	oMenu.makeSub(posicao,8,'Demanda',0,qtd_6)
	oMenu.makeSub(posicao,9,'Grade',0,qtd_6)
	oMenu.makeSub(posicao,10,'Turma Corte 1',0,qtd_6)
	oMenu.makeSub(posicao,11,'Compara Unidades',raiz + '/asp/grade/param_unidade_x_unidade.asp?pNumRel=1&pTituloRel=Compara Unidades por Corte',qtd_6)
	oMenu.makeSub(posicao,12,'Auditoria',0,qtd_6)

	pos_Sub = 1
	qtd_60 = 2
	oMenu.makeSubSub(posicao,pos_Sub,0,'Unidades',raiz + '/asp/grade/consulta_unidade.asp?pNumRel=1&pTituloRel=Unidade',qtd_60)
	oMenu.makeSubSub(posicao,pos_Sub,1,'Unidade x Orgão',raiz + '/asp/grade/consulta_unidade.asp?pNumRel=2&pTituloRel=Unidade x Orgão',qtd_60)

	pos_Sub = 7
	qtd_60 = 1
	oMenu.makeSubSub(posicao,pos_Sub,0,'Orgão Maior x Menor',raiz + '/asp/grade/param_orgao_maior_menor.asp?pNumRel=1&pTituloRel=Orgao Maior x Menor',qtd_60)

	pos_Sub = 8
	qtd_60 = 2
	//oMenu.makeSubSub(posicao,pos_Sub,0,'Demanda x Oferta EAD',raiz + '/asp/grade/consulta_geral.asp?pNumRel=1&pTituloRel=Demanda x Oferta EAD',qtd_60)
	oMenu.makeSubSub(posicao,pos_Sub,0,'Demanda Curso',raiz + '/asp/grade/consulta_geral.asp?pNumRel=2&pTituloRel=Demanda Curso',qtd_60)
	//oMenu.makeSubSub(posicao,pos_Sub,1,'Demanda x Oferta',raiz + '/asp/grade/param_demanda_x_turma_corte.asp?pNumRel=2&pTituloRel=Demanda x Oferta',qtd_60)
	oMenu.makeSubSub(posicao,pos_Sub,1,'Demanda x Oferta',raiz + '/asp/grade/dados_demanda_x_turma_corte.asp',qtd_60)
	
	pos_Sub = 9
	qtd_60 = 1
	oMenu.makeSubSub(posicao,pos_Sub,0,'Grade de Treinamento',raiz + '/asp/grade/consulta_grade.asp?pTituloRel=Grade de Treinamento',qtd_60)

	pos_Sub = 10
	qtd_60 = 1
	oMenu.makeSubSub(posicao,pos_Sub,0,'Qtd Turmas / Mes',raiz + '/asp/grade/dados_qtd_turmas_por_mes.asp',qtd_60)
