...
'response.Write("Macro1" & request("txtSeqMacro1"))
'response.Write("Macro2" & request("txtSeqPerfil1"))
'response.Write("desc atual =" & request("txtDescPerfil1"))
'response.Write("desc antiga=" & request("txtObsAnterior1"))
'response.End()

do while int_contador <= (int_QtdObj+1)-1
	int_SeqMacro = request("txtSeqMacro" & int_contador) 
	int_SeqMicro = request("txtSeqPerfil" & int_contador) 
	str_Desc_Atual =  request("txtDescPerfil" & int_contador)
	...
	if str_Desc_Atual <> str_Desc_Anterior then	
	   call f_Altera_Desc(int_SeqMacro,int_SeqMicro,str_Desc_Atual)
	end if   
	int_contador = int_contador + 1
	'response.write int_contador
loop

Sub f_Altera_Desc(p_SeqMacro,p_SeqMicro, p_Desc)
    int_Qtd_Alterado = int_Qtd_Alterado + 1
    ssql=" UPDATE " & Session("PREFIXO") & "MICRO_PERFIL_R3 SET MIPE_TX_ORIENTACAO = '" & p_Desc & "'"	
	ssql=ssql & " ,ATUA_TX_OPERACAO = 'A'"   
	ssql=ssql & " ,ATUA_CD_NR_USUARIO = '" & Session("CdUsuario") & "'"
	ssql=ssql & " ,ATUA_DT_ATUALIZACAO = GETDATE()"
	ssql=ssql & " WHERE MCPR_NR_SEQ_MACRO_PERFIL = " & p_SeqMacro 
	ssql=ssql & " AND MIPE_NR_SEQ_MICRO_PERFIL = " & p_SeqMicro 
