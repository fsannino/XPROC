...
function Confirma()
{
  ...
  if (document.frm1.selOrient.selectedIndex == 0)
     { 
	 alert("A seleção de uma Orientação é obrigatória!");
     document.frm1.selOrient.focus();
     return;
     }      
   if(document.frm1.txtOpt.value == "A")
     {
     document.frm1.action="alterar_excluir_ori_gerais_mape_perfil.asp";
...
<form name="frm1" method="POST" action="">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr> 
      <td width="21%">&nbsp;</td>
      <td width="53%"><div align="center"><font size="3" face="Verdana, Arial, Helvetica, sans-serif" color="#000099">Seleciona 
          orienta&ccedil;&otilde;es ao mapeamento de Perfil - <%=str_Titulo%></font></div></td>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr> 
      <td><div align="right"><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#003366"><b>Selecione 
          Orienta&ccedil;&atilde;o :</b></font></div></td>
      <td><select name="selOrient" size="1" id="selOrient">
          <option value="0">== Selecione uma Orientação ==</option>
          <% set rs=db.execute(str_SQL)
		     do until rs.eof=true %>
               <option value="<%=rs("ORIE_NR_SEQUENCIAL")%>"><%=rs("ORIE_NR_SEQUENCIAL")%> - <%=Left(rs("ORIE_TX_ORIENTACOES"),90)%></option>
		       <% rs.movenext
			loop %>
        </select></td>
      <td>&nbsp;</td>
