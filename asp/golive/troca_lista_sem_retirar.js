...
function move_tudo(fbox,tbox)
{
	var items = fbox.length;	
...
function move(fbox,tbox,bmsg) 
{
	for(var i=0; i<fbox.options.length; i++) 
	{
		if(fbox.options[i].selected && fbox.options[i].value != "") 
		{
			if(JaExiste(fbox.options[i].value,tbox,bmsg))
			{
			var no = new Option();
...
function JaExiste(objvalor,tbox,bmsg) 
{
