...
response.End()
	...
	str_SQL = str_SQL & " AND SUBSTRING(USMA_CD_USUARIO,1,1) IN ('A','B','C')"
...
Sub f_grava_registro_curso(str_Cd_Usu_Anterior, str_Cd_Curso, str_Status)
    ...
    If rstRepeticao.EOF Then
		...
		if str_Status = "AP" then
			if Trim(rstRepeticao("USAP_TX_APROVEITAMENTO")) = " " or Trim(rstRepeticao("USAP_TX_APROVEITAMENTO")) = "LM" then
				str_SQL = ""
...
response.Redirect("importa_usuario2.asp")
%>
<html>
<head>
<script>
