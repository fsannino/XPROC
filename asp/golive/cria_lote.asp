...
str_SQL = str_SQL & " order by USMA_CD_USUARIO , FUNE_CD_FUNCAO_NEGOCIO  "
    ...
    'str_SQL = str_SQL & " Where USMA_CD_USUARIO > '0'"
    'str_SQL = str_SQL & " AND FUUS_IN_VALIDADO = 'S'"
    'str_SQL = str_SQL & " AND (FUNE_CD_FUNCAO_NEGOCIO = 'BW.166'"
    'str_SQL = str_SQL & " or FUNE_CD_FUNCAO_NEGOCIO = 'BW.182'"
    'str_SQL = str_SQL & " or FUNE_CD_FUNCAO_NEGOCIO = 'BW.165'"
    'str_SQL = str_SQL & " or FUNE_CD_FUNCAO_NEGOCIO = 'BW.073')"
    'str_SQL = str_SQL & " AND (USMA_CD_USUARIO = 'XK23'"
    'str_SQL = str_SQL & " OR USMA_CD_USUARIO = 'XR93'"
    'str_SQL = str_SQL & " OR USMA_CD_USUARIO = 'XK45')"
    'str_SQL = str_SQL & " order by USMA_CD_USUARIO , FUNE_CD_FUNCAO_NEGOCIO  "
...
If int_NumReg_Usu_Curso > 0 Then
	...
	Do Until int_NumReg_Usu_Curso = int_Loop_Usu_Curso
		...
		str_Cd_Usu_Anterior <> rds_Usu_Curso("USMA_CD_USUARIO")
			...
			str_Cd_Fun_Anterior <> rds_Usu_Curso("FUNE_CD_FUNCAO_NEGOCIO")
                ...
                If int_NumReg_TabIncr > 0 Then
                    Do Until int_NumReg_TabIncr = int_Loop_TabIncr
                        ...
                        End If
                        If rds_TabIncr("USAP_TX_APROVEITAMENTO") = "LM" Then
                            lng_Posicao = SequentialSearchStringArray(vet_Fun_Definitiva, str_Cd_Fun_Anterior)
                            str_Teste_Ant_Def = 0
                            If lng_Posicao <> -1 Then
                                str_Cd_Fun_Anterior_2 = vet_Fun_Antecipada(lng_Posicao)
                                str_Teste_Ant_Def = 1
                            End If
                            If str_Teste_Ant_Def = 1 Then
                                ...
                                str_SQL = str_SQL & " , dbo.USUARIO_APROVADO.USAP_CD_USUARIO"
                                str_SQL = str_SQL & " , dbo.USUARIO_APROVADO.USAP_TX_APROVEITAMENTO"
                                str_SQL = str_SQL & " FROM dbo.CURSO_FUNCAO INNER JOIN"
                                str_SQL = str_SQL & " dbo.USUARIO_APROVADO ON"
                                str_SQL = str_SQL & " dbo.CURSO_FUNCAO.CURS_CD_CURSO = dbo.USUARIO_APROVADO.CURS_CD_CURSO"
                                str_SQL = str_SQL & " WHERE  "
                                str_SQL = str_SQL & " dbo.CURSO_FUNCAO.FUNE_CD_FUNCAO_NEGOCIO = '" & str_Cd_Fun_Anterior_2 & "'"
                                str_SQL = str_SQL & " AND dbo.USUARIO_APROVADO.USAP_CD_USUARIO = '" & str_Cd_Usu_Anterior & "'"
                                Set rds_TabIncrAnt = conn_Cogest.Execute(str_SQL)
                                int_NumReg_TabIncrAnt = rds_TabIncrAnt.RecordCount
                                int_Loop_TabIncrAnt = 0
                                If int_NumReg_TabIncrAnt > 0 Then
                                    Do Until int_NumReg_TabIncrAnt = int_Loop_TabIncrAnt
                                        If rds_TabIncrAnt("USAP_TX_APROVEITAMENTO") = "AP" Or rds_TabIncrAnt("USAP_TX_APROVEITAMENTO") = "LM" Then
                                            int_Aprovado = int_Aprovado + 1
                                        Else
                                            int_Nao_Aprovado = int_Nao_Aprovado + 1
                                        End If
                                        int_Loop_TabIncrAnt = int_Loop_TabIncrAnt + 1
                                        rds_TabIncrAnt.MoveNext
                                    Loop
                                Else
                                    int_Nao_Aprovado = int_Nao_Aprovado + 1
                                End If
			...
			'****************************
			If int_Nao_Aprovado = 0 Then
				Select case str_MegaProcesso
					case  12
						Call f_grava_registro_RH(str_Cd_Usu_Anterior, str_Cd_Fun_Anterior, "AP")					
					case  15
						Call f_grava_registro_BW(str_Cd_Usu_Anterior, str_Cd_Fun_Anterior, "AP")										
					case else
						Call f_grava_registro(str_Cd_Usu_Anterior, str_Cd_Fun_Anterior, "AP", str_Teste_Ant_Def)					
				End select
				'if str_MegaProcesso <> 12 then
...
Sub f_grava_registro (str_Cd_Usu_Anterior,str_Cd_Fun_Anterior,str_Status, str_Prev_Ant)

'	RESPONSE.Write(str_MegaProcesso)
'	RESPONSE.End()

		str_SQL = " SELECT DISTINCT "
        str_SQL = str_SQL & " dbo.FUNCAO_USUARIO_PERFIL.FUNE_CD_FUNCAO_NEGOCIO"
        str_SQL = str_SQL & " , dbo.FUNCAO_USUARIO_PERFIL.USMA_CD_USUARIO"
        str_SQL = str_SQL & " , dbo.MICRO_PERFIL_R3.MCPR_NR_SEQ_MACRO_PERFIL"
		...
		' grava quando possuir algum perfil associado
		if not rds_Perfil.Eof then
			...
			do while not rds_Perfil.Eof
				...
				str_SQL = str_SQL & " select "
				str_SQL = str_SQL & " FUNE_CD_FUNCAO_NEGOCIO "
				str_SQL = str_SQL & " FROM GOLI_FUNCAO_USUARIO_COM_PERFIL "
				str_SQL = str_SQL & " WHERE USMA_CD_USUARIO = '" & str_Cd_Usu_Anterior & "'"
				str_SQL = str_SQL & " AND FUNE_CD_FUNCAO_NEGOCIO = '" & str_Cd_Fun_Anterior & "'"
				str_SQL = str_SQL & " AND MCPR_NR_SEQ_MACRO_PERFIL = " & rds_Perfil("MCPR_NR_SEQ_MACRO_PERFIL")
				str_SQL = str_SQL & " AND MIPE_NR_SEQ_MICRO_PERFIL = " & rds_Perfil("MIPE_NR_SEQ_MICRO_PERFIL") 					
				...
				If rstRepeticao.EOF Then
					...
					End If
					str_SQL = ""
					str_SQL = str_SQL & " Insert into GOLI_FUNCAO_USUARIO_COM_PERFIL("
					str_SQL = str_SQL & " LOTE_NR_SEQ_LOTE"
					str_SQL = str_SQL & " , USMA_CD_USUARIO"
					str_SQL = str_SQL & " , FUNE_CD_FUNCAO_NEGOCIO"
				'	str_SQL = str_SQL & " , LOTE_NR_ANT_DEF"
				...
				'	str_SQL = str_SQL & " " & str_Prev_Ant & ","
					str_SQL = str_SQL &  rds_Perfil("MCPR_NR_SEQ_MACRO_PERFIL") & ","
					str_SQL = str_SQL &  rds_Perfil("MIPE_NR_SEQ_MICRO_PERFIL") & ","	
		...
		' grava quando não possuir algum perfil associado
	
			str_SQL = ""
			str_SQL = str_SQL & " select "
			str_SQL = str_SQL & " FUNE_CD_FUNCAO_NEGOCIO "
			str_SQL = str_SQL & " FROM GOLI_FUNCAO_USUARIO_SEM_PERFIL "
			str_SQL = str_SQL & " WHERE USMA_CD_USUARIO = '" & str_Cd_Usu_Anterior & "'"
			str_SQL = str_SQL & " AND FUNE_CD_FUNCAO_NEGOCIO = '" & str_Cd_Fun_Anterior & "'"
			set rstRepeticao = conn_Cogest.Execute(str_SQL)
			If rstRepeticao.EOF Then		
				...
				str_SQL = ""
				str_SQL = str_SQL & " Insert into GOLI_FUNCAO_USUARIO_SEM_PERFIL("
				str_SQL = str_SQL & " LOTE_NR_SEQ_LOTE"
				str_SQL = str_SQL & " , USMA_CD_USUARIO"
				str_SQL = str_SQL & " , FUNE_CD_FUNCAO_NEGOCIO"
			'	str_SQL = str_SQL & " , LOTE_NR_ANT_DEF"
...
Sub f_grava_registro_RH (str_Cd_Usu_Anterior,str_Cd_Fun_Anterior,str_Status)

	'RESPONSE.Write("RH  -  " & str_MegaProcesso)
	'RESPONSE.End()

		str_SQL = " SELECT DISTINCT "
		str_SQL = str_SQL & " dbo.MICRO_PERFIL_R3.MCPR_NR_SEQ_MACRO_PERFIL, dbo.MICRO_PERFIL_R3.MIPE_NR_SEQ_MICRO_PERFIL, "
		str_SQL = str_SQL & " dbo.FUNCAO_USUARIO_PERFIL_VISAO.VISA_NR_SEQ_VISAO"
		str_SQL = str_SQL & " FROM dbo.MICRO_PERFIL_R3 INNER JOIN"
		str_SQL = str_SQL & " dbo.FUNCAO_USUARIO_PERFIL_VISAO ON "
		str_SQL = str_SQL & " dbo.MICRO_PERFIL_R3.MCPR_NR_SEQ_MACRO_PERFIL = dbo.FUNCAO_USUARIO_PERFIL_VISAO.MCPR_NR_SEQ_MACRO_PERFIL AND "
		str_SQL = str_SQL & " dbo.MICRO_PERFIL_R3.MIPE_NR_SEQ_MICRO_PERFIL = dbo.FUNCAO_USUARIO_PERFIL_VISAO.MIPE_NR_SEQ_MICRO_PERFIL"
		str_SQL = str_SQL & " WHERE dbo.FUNCAO_USUARIO_PERFIL_VISAO.FUNE_CD_FUNCAO_NEGOCIO = '" & str_Cd_Fun_Anterior & "'"
		str_SQL = str_SQL & " AND dbo.FUNCAO_USUARIO_PERFIL_VISAO.USMA_CD_USUARIO = '" & str_Cd_Usu_Anterior & "'"
		str_SQL = str_SQL & " AND dbo.FUNCAO_USUARIO_PERFIL_VISAO.FUPV_IN_VALIDADO = 'S'"
		'if str_Cd_Usu_Anterior = "CM75" and str_Cd_Fun_Anterior = "HR.05" THEN
		'	response.Write(str_SQL)
		'	response.End()
		'end if
		set rds_Perfil = conn_Cogest.Execute(str_SQL)
		...
		if not rds_Perfil.Eof then
			...
			do while not rds_Perfil.Eof
				...
				str_SQL = str_SQL & " select "
				str_SQL = str_SQL & " FUNE_CD_FUNCAO_NEGOCIO "
				str_SQL = str_SQL & " FROM GOLI_FUNCAO_USUARIO_COM_PERFIL_RH "
				str_SQL = str_SQL & " WHERE USMA_CD_USUARIO = '" & str_Cd_Usu_Anterior & "'"
				str_SQL = str_SQL & " AND FUNE_CD_FUNCAO_NEGOCIO = '" & str_Cd_Fun_Anterior & "'"
				str_SQL = str_SQL & " AND MCPR_NR_SEQ_MACRO_PERFIL = " & rds_Perfil("MCPR_NR_SEQ_MACRO_PERFIL")
				str_SQL = str_SQL & " AND MIPE_NR_SEQ_MICRO_PERFIL = " & rds_Perfil("MIPE_NR_SEQ_MICRO_PERFIL") 		
				str_SQL = str_SQL & " AND VISA_NR_SEQ_VISAO = " & rds_Perfil("VISA_NR_SEQ_VISAO") 		
				...
				If rstRepeticao.EOF Then
					...
					End If
					str_SQL = ""
					str_SQL = str_SQL & " Insert into GOLI_FUNCAO_USUARIO_COM_PERFIL_RH("
					str_SQL = str_SQL & " LOTE_NR_SEQ_LOTE"
					str_SQL = str_SQL & " , USMA_CD_USUARIO"
					str_SQL = str_SQL & " , FUNE_CD_FUNCAO_NEGOCIO"
					str_SQL = str_SQL & " , MCPR_NR_SEQ_MACRO_PERFIL"
					str_SQL = str_SQL & " , MIPE_NR_SEQ_MICRO_PERFIL"
					str_SQL = str_SQL & " , VISA_NR_SEQ_VISAO"						
					str_SQL = str_SQL & " , ATUA_TX_OPERACAO"
					str_SQL = str_SQL & " , ATUA_CD_NR_USUARIO"
					str_SQL = str_SQL & " , ATUA_DT_ATUALIZACAO"
					str_SQL = str_SQL & " )Values("
					...
					str_SQL = str_SQL &  rds_Perfil("MCPR_NR_SEQ_MACRO_PERFIL") & ","
					str_SQL = str_SQL &  rds_Perfil("MIPE_NR_SEQ_MICRO_PERFIL") & ","	
					str_SQL = str_SQL &  rds_Perfil("VISA_NR_SEQ_VISAO") & ","							
					str_SQL = str_SQL & "'C' ,'" & Session("CdUsuario") & "' ,GETDATE())"
					'response.Write(str_SQL)
					'response.End()
...
Sub f_grava_registro_BW (str_Cd_Usu_Anterior,str_Cd_Fun_Anterior,str_Status)
...
'	RESPONSE.End()
		...
		' grava quando possuir algum perfil associado
		if not rds_Perfil.Eof then
			int_Seq_macro = rds_Perfil("MCPR_NR_SEQ_MACRO_PERFIL")
			do while not rds_Perfil.Eof
			...
			Loop
			'response.Write(str_Cd_Usu_Anterior)
			'response.Write(str_Cd_Fun_Anterior)
			str_SQL = ""
			str_SQL = str_SQL & " SELECT     "
			str_SQL = str_SQL & "   MCPR_NR_SEQ_MACRO_PERFIL"
			str_SQL = str_SQL & " , MCPE_TX_NOME_DERIVACAO"
			str_SQL = str_SQL & " , MCPE_BO_DERIVACAO"
			str_SQL = str_SQL & " , MEPR_CD_MEGA_PROCESSO"
			str_SQL = str_SQL & " FROM dbo.MACRO_PERFIL"
			str_SQL = str_SQL & " WHERE MCPR_NR_SEQ_MACRO_PERFIL = "  & int_Seq_macro
			
			Set rdsPossuiDeriva = conn_Cogest.Execute(str_SQL)
			if not rdsPossuiDeriva.EOF then
				if not IsNull(rdsPossuiDeriva("MCPE_TX_NOME_DERIVACAO")) and rdsPossuiDeriva("MCPE_TX_NOME_DERIVACAO") <> 0 then
					
					'str_SQL = ""
					'str_SQL = str_SQL & " SELECT "
					'str_SQL = str_SQL & " dbo.MACRO_PERFIL.MCPR_NR_SEQ_MACRO_PERFIL, dbo.MACRO_PERFIL.MCPE_TX_NOME_DERIVACAO, "
					'str_SQL = str_SQL & " dbo.MACRO_PERFIL.MCPE_BO_DERIVACAO, dbo.MACRO_PERFIL.MEPR_CD_MEGA_PROCESSO, "
					'str_SQL = str_SQL & " dbo.MICRO_PERFIL_R3.MIPE_NR_SEQ_MICRO_PERFIL, "
					'str_SQL = str_SQL & " dbo.MICRO_PERFIL_R3.MIPE_TX_NOME_TECNICO"
					'str_SQL = str_SQL & " FROM dbo.MACRO_PERFIL INNER JOIN"
					...
					'str_SQL = str_SQL & " WHERE MCPR_NR_SEQ_MACRO_PERFIL  = "  & rdsPossuiDeriva("MCPE_TX_NOME_DERIVACAO")
                    ...
                    str_SQL = str_SQL & " dbo.FUNCAO_USUARIO_PERFIL.MIPE_NR_SEQ_MICRO_PERFIL = dbo.MICRO_PERFIL_R3.MIPE_NR_SEQ_MICRO_PERFIL"
                    str_SQL = str_SQL & " WHERE "
                    str_SQL = str_SQL & " dbo.FUNCAO_USUARIO_PERFIL.MCPR_NR_SEQ_MACRO_PERFIL  = " & rdsPossuiDeriva("MCPE_TX_NOME_DERIVACAO")
                    str_SQL = str_SQL & " AND dbo.FUNCAO_USUARIO_PERFIL.USMA_CD_USUARIO = '" & str_Cd_Usu_Anterior & "'"
					'response.Write(str_SQL)					
					
					Set rdsDeriva = conn_Cogest.Execute(str_SQL)
					if not rdsDeriva.EOF then
						do while not rdsDeriva.EOF
							Call f_grava_Micro_BW(str_Cd_Usu_Anterior, str_Cd_Fun_Anterior, rdsDeriva("MCPR_NR_SEQ_MACRO_PERFIL"), rdsDeriva("MIPE_NR_SEQ_MICRO_PERFIL"))												
							rdsDeriva.MoveNext							
						loop
					end if 
				end if
...
Sub f_grava_Micro_BW(str_Cd_Usu_Anterior, str_Cd_Funcao, str_Cd_Macro, str_Cd_Micro)
	...
	str_SQL = str_SQL & "'" & str_Cd_Usu_Anterior & "',"
	str_SQL = str_SQL & "'" & str_Cd_Fun_Anterior & "',"
	str_SQL = str_SQL &  str_Cd_Macro & ","
	str_SQL = str_SQL &  str_Cd_Micro & ","	
	str_SQL = str_SQL & "'C' ,'" & Session("CdUsuario") & "' ,GETDATE())"
	'response.Write(str_SQL)
	Set rdsNovo = conn_Cogest.Execute(str_SQL)
...
%>
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
      <td>&nbsp;</td>
		<% if int_Num_Lote > 0 then %>
      <td><a href="consulta_lote2.asp"><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Ver lote</font></a> </td>
		<% end if %>
      <td>&nbsp;</td>
    </tr>
    <tr>
