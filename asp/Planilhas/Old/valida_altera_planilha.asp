...
ssql_d = "DELETE FROM " & planil
db.execute(ssql_d)

set rs=db.execute("SELECT * FROM " & planil)
...
do until contador = linhas + 1
	...
	ssql=""
	ssql="INSERT INTO " & planil
	
	valor_campo_cod = REQUEST(RS.FIELDS(CONTA_CAMPO).NAME & "_" & CONTADOR)
	
	'response.write valor_campo_cod

	if valor_campo_cod="" then
		valor_campo_cod=0
	end if
	
	ssql=ssql&" VALUES (" & valor_campo_cod & ","
	
	CONTA_CAMPO = CONTA_CAMPO + 1
	
	DO UNTIL CONTA_CAMPO=VALOR_CAMPOS
		ssql=ssql&"'" & REQUEST(RS.FIELDS(CONTA_CAMPO).NAME & "_" & CONTADOR) & "',"
		CONTA_CAMPO = CONTA_CAMPO + 1 
	LOOP
	...
	'response.write ssql
	db.execute(ssql)
	'call grava_log(valor_campo_cod,planil,"I",1)
loop

stat="Os Registros modificados foram alterados com Sucesso"
cor="#330099"
%>
<html>
...
<p align="center">
<font face="Verdana" size="3" color="#330099">Alteração de Linhas</font>
