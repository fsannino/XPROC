<%@LANGUAGE="VBSCRIPT"%> 
<%
caminho=server.mappath("../../publico/golive/arquivo.txt")
response.write caminho

set fso = server.CreateObject("Scripting.FileSystemObject")
set arquivo = fso.CreateTextFile(caminho)
...
function chamapagina()
	{
	window.open('importacao.asp?par_PaginaPrint=exporta_usuario.asp','jan1','toolbar=no, location=no, scrollbars=no, status=no, directories=no, resizable=no, menubar=no, fullscreen=no, height=50, width=250, status=no, top=200, left=260');
	}

	setTimeout('chamapagina()',1);
