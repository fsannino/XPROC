...
IF MEGA<>0 THEN
COMPL1="dbo.FUNCAO_NEGOCIO.MEPR_CD_MEGA_PROCESSO=" & MEGA
END IF
...
str_Sql = str_Sql & " , dbo.FUN_NEG_TRANSACAO.PROC_CD_PROCESSO"
str_Sql = str_Sql & " , dbo.FUN_NEG_TRANSACAO.SUPR_CD_SUB_PROCESSO"
str_Sql = str_Sql & " , dbo.FUN_NEG_TRANSACAO.ATCA_CD_ATIVIDADE_CARGA"
str_Sql = str_Sql & " , dbo.FUN_NEG_TRANSACAO.MODU_CD_MODULO"
...
str_Sql = str_Sql & " dbo.TRANSACAO ON dbo.FUN_NEG_TRANSACAO.TRAN_CD_TRANSACAO = dbo.TRANSACAO.TRAN_CD_TRANSACAO LEFT OUTER JOIN"
...
str_Sql = str_Sql & " ORDER BY dbo.FUNCAO_NEGOCIO.MEPR_CD_MEGA_PROCESSO, dbo.FUN_NEG_TRANSACAO.PROC_CD_PROCESSO, "
str_Sql = str_Sql & " dbo.FUN_NEG_TRANSACAO.SUPR_CD_SUB_PROCESSO, dbo.FUN_NEG_TRANSACAO.ATCA_CD_ATIVIDADE_CARGA, "
str_Sql = str_Sql & " dbo.FUN_NEG_TRANSACAO.MODU_CD_MODULO"
...
<table border="0" width="100%">
  <tr>
    ...
    <td width="16%" bgcolor="#B5D6E8"><b><font face="Verdana" size="1">Função</font></b></td>	
    <td width="16%" bgcolor="#B5D6E8"><b><font face="Verdana" size="1">Dono</font></b></td>
...
funcao_atual=RS("FUNE_CD_FUNCAO_NEGOCIO")
...
do until rs.eof=true
	...
	if mega_dono_ant<>mega_dono_atual then
		...
		valor7=rs1("MEPR_TX_DESC_MEGA_PROCESSO")
	else
		valor7=""
	end if

	if funcao_ant<>funcao_atual then
		'set rs1=db.execute("SELECT * FROM " & Session("PREFIXO") & "MEGA_PROCESSO WHERE MEPR_CD_MEGA_PROCESSO='" & funcao_atual & "'")
		valor0 = rs("FUNE_CD_FUNCAO_NEGOCIO") & "-" & rs("FUNE_TX_TITULO_FUNCAO_NEGOCIO")
	else
		valor0=""
	end if
...
if proc_ant<>proc_atual then
	...
	if not rs1.Eof then
		...
		str_ = "SELECT * FROM " & Session("PREFIXO") & "PROCESSO WHERE MEPR_CD_MEGA_PROCESSO=" & mega_atual & " AND PROC_CD_PROCESSO=" & proc_atual
		response.Write(str_)
		valor2 = " "
	end if	
else
...
%>
  <tr>
    ...
    <td width="17%" bgcolor="#FFFF00"><font face="Verdana" size="1"><%=valor0%></font></td>
...
<%
	funcao_ant=RS("FUNE_CD_FUNCAO_NEGOCIO")
