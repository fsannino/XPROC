...
set rs2=db.execute("SELECT * FROM " & Session("PREFIXO") & "ATIVIDADE WHERE MEPR_CD_MEGA_PROCESSO="& request("selMegaProcesso")& "ORDER BY ATIV_TX_DESC_ATIVIDADE")



set rs4=db.execute("SELECT * FROM " & Session("PREFIXO") & "PROCESSO ORDER BY PROC_TX_DESC_PROCESSO")

set rs5=db.execute("SELECT * FROM " & Session("PREFIXO") & "SUB_PROCESSO WHERE PROC_CD_PROCESSO="& request("SelProcesso"))
%>
...
<form method="POST" action="resultado_consulta.asp" name="formulario">
  ...
  <option value=0>== Selecione ==</option>
    <%
  	 ...
  	 response.write request("selMegaProcesso")
  	 response.write rs("MEPR_CD_MEGA_PROCESSO")
  	 IF trim(request("selMegaProcesso"))= trim(rs("MEPR_CD_MEGA_PROCESSO"))THEN
    %>  
  ...
  a Atividade&nbsp;&nbsp;&nbsp; <select size="1" name="selAtividade" onChange="javascript:goToURL2()">
    ...
    <%
  	 ...
  	 IF (trim(request("selAtividade"))= trim(rs2("ATIV_CD_ATIVIDADE")))THEN
  	 %>
  ...
  o Processo&nbsp; <select size="1" name="selProcesso" onChange="javascript:goToURL3()">
    ...
    if trim(request("selProcesso"))=trim(rs4("PROC_CD_PROCESSO")) then
    %>
    <option selected value=<%=rs4("PROC_CD_PROCESSO")%>><%=RS4("PROC_TX_DESC_PROCESSO")%></option>
    <%else%>
  ...
  o Sub-Processo<b>&nbsp; </b><select size="1" name="selSubProcesso">
    ...
    %>
    <option value=<%=RS5("SUPR_CD_SUB_PROCESSO")%>><%=RS5("SUPR_TX_DESC_SUB_PROCESSO")%></option>
    <%
    RS5.MOVENEXT
    LOOP
    %>
  ...
  <p style="margin-top: 0; margin-bottom: 0">&nbsp;</p>
  <p style="margin-top: 0; margin-bottom: 0"><font face="Arial" size="2"><b>Novo
  Sub-Processo&nbsp; <input type="text" name="novo_subprocesso" size="34"></b></font></p>
  <p style="margin-top: 0; margin-bottom: 0">&nbsp;</p>
