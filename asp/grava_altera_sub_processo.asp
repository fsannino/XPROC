...
Sub Grava_Sub_Processo(ls_Desc_SubProceso, ls_Seq)
    str_SQL_Sub_Proc = ""
	...
	if impacto<>0 then
		str_SQL_Sub_Proc = str_SQL_Sub_Proc & " ,SUPR_TX_IMPACTO = " & impacto
	end if
   ...
   strChave = CStr(ls_Seq) 
   'call grava_log(strChave,"SUB_PROCESSO","A",1)
...
Sub Grava_Novo_Sub_Processo_Empresa(strMP, strP, strSP, strEU)
	...
	strChave = CStr(strMP) & " " & CStr(strP) & " " & CStr(strSP) & " " & CStr(strEU) ' & " " & CStr(strA) & CStr(strT)
...
if str_Desc_SubProceso <> "" then
   call Grava_Sub_Processo(Trim(str_Desc_SubProceso),str_Seq)
end if
