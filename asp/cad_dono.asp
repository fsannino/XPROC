...
ssql = ""
ssql="SELECT * FROM " & Session("PREFIXO") & "TRANSACAO ORDER BY TRAN_CD_TRANSACAO"

SET RS=CONN_DB.EXECUTE(SSQL)

RS_MEGA = "SELECT * FROM " & Session("PREFIXO") & "MEGA_PROCESSO ORDER BY MEPR_TX_DESC_MEGA_PROCESSO"

set rs_destino=conn_db.execute("SELECT * FROM " & Session("PREFIXO") & "TRANSACAO_MEGA WHERE TRAN_CD_TRANSACAO='" & request("transacao") & "'")
%>
...
function manda()
{
window.location.href="cad_dono.asp?transacao="+document.frm1.selTransacao.value
}
...
<form name="frm1" method="post" action="valida_cad_dono.asp">
  ...
  <table width="90%" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr> 
      <td width="6%">&nbsp;</td>
      <td width="26%">&nbsp;</td>
      <td width="68%"><font color="#003366" size="3" face="Verdana, Arial, Helvetica, sans-serif">Cadastro
        de Dono de Transação</font></td>
    </tr>
    <tr> 
      <td width="6%">&nbsp;</td>
      <td width="26%">&nbsp;</td>
    ...
    <tr> 
      <td width="6%">&nbsp;</td>
      <td width="26%"><div align="right"><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#003366"><b>Transa&ccedil;&otilde;es 
          : </b></font></div></td>
      <td width="68%"> 
        <select name="selTransacao" size="1" onchange="javascript:manda()">
          <option value="0" selected>Selecione uma Transação</option>
          <%DO UNTIL RS.EOF=TRUE
          if trim(request("transacao"))=trim(rs("TRAN_CD_TRANSACAO")) THEN
          %>
          <option selected value=<%=RS("TRAN_CD_TRANSACAO")%>><%=RS("TRAN_CD_TRANSACAO")%>-<%=LEFT(RS("TRAN_TX_DESC_TRANSACAO"),55)%></option>
          <%
          ELSE
          %>
          <option value=<%=RS("TRAN_CD_TRANSACAO")%>><%=RS("TRAN_CD_TRANSACAO")%>-<%=LEFT(RS("TRAN_TX_DESC_TRANSACAO"),55)%></option>
			<%
			end if
			RS.MOVENEXT
			LOOP
    ...
    <tr> 
      <td width="6%">&nbsp;</td>
      <td width="26%">&nbsp;</td>
      <td width="68%"><table width="87%" border="0">
          <tr> 
            <td><div align="center">N&atilde;o selecionadas</div></td>
            <td><div align="center">Selecionadas</div></td>
          </tr>
        </table> </td>
    </tr>
    <tr> 
      <td width="6%">&nbsp;</td>
      <td width="26%" valign="top"><div align="right"><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#003366"><b>Mega 
          / Dono : </b></font></div></td>
      <td width="68%"> 
        <table width="12%" border="0" align="left" cellpadding="0" cellspacing="0">
          <tr> 
            <td width="11%"> 
              <div align="center"> <b> 
                <select name="list1" size="8" multiple>
                  <%
                 Set RS1 = Conn_db.Execute(RS_MEGA)
                 DO UNTIL RS1.EOF=TRUE
                 
                 set rstemp=conn_db.execute("SELECT * FROM " & Session("PREFIXO") & "TRANSACAO_MEGA WHERE TRAN_CD_TRANSACAO='" & REQUEST("TRANSACAO") & "' AND MEPR_CD_MEGA_PROCESSO=" & RS1("MEPR_cD_MEGA_PROCESSO"))
            ...
            <td width="5%" align="center"> 
              <table width="2%" cellpadding="0" cellspacing="0" border="0" align="center">
                <tr> 
            ...
            <td width="84%"> 
              <div align="center"><font color="#000080"> 
                ...
                set temp=conn_db.execute("SELECT * FROM " & Session("PREFIXO") & "MEGA_PROCESSO WHERE MEPR_cD_MEGA_PROCESSO=" & RS_destino("MEPR_CD_MEGA_PROCESSO"))
                VALOR=TEMP("MEPR_TX_DESC_MEGA_PROCESSO")
                 	%>
                  <option value="<%=RS_destino("MEPR_CD_MEGA_PROCESSO")%>" ><%=valor%></option>
                 <%
                 	RS_destino.MoveNext
					LOOP
					%>
