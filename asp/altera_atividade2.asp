...
<%
str_cod = Request("CodAtividade")
str_seq = request("sequencia")
str_Param = request("DescAtividade")

set db = Server.CreateObject("ADODB.Connection")
db.Open Session("Conn_String_Cogest_Gravacao")

if len(str_seq)=0 then
	str_seq=0
end if

ssql=""
ssql="UPDATE " & Session("PREFIXO") & "ATIVIDADE_CARGA "
ssql=ssql & "SET ATCA_TX_DESC_ATIVIDADE='" & str_Param & "', "
ssql=ssql & "ATCA_NR_SEQUENCIA=" & str_seq & " "
ssql=ssql & "WHERE ATCA_CD_ATIVIDADE_CARGA=" & str_cod
...
db.execute(ssql)
'call grava_log(str_cod,"" & Session("PREFIXO") & "ATIVIDADE_CARGA","A",1)
...
<form method="POST" action="valida_altera_processo.asp?mega=<%=str_MegaProcesso%>&Proc=<%=str_Processo%>">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr> 
      <td width="3%"></td>
      <td width="24%"></td>
      <td width="59%"><font face="Verdana, Arial, Helvetica, sans-serif" color="#000099" size="2"><b>Atividade
        Atualizada com Sucesso!</b></font></td>
      <td width="14%"></td>
    </tr>
    <%else%>
    <tr> 
