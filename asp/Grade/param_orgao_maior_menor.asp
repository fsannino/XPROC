...
'response.Write(Request("selDiretoria"))
'response.Write(" ------ ")
...
if trim(request("selOrgaoMaior")) <> "" then
	strCdOrgaoMaior = trim(request("selOrgaoMaior"))
else
	strCdOrgaoMaior = "0"
end if
...
str_Sql = str_Sql & " dbo.GRADE_ORGAO_MAIOR"
str_Sql = str_Sql & " WHERE "
str_Sql = str_Sql & " CORT_CD_CORTE = " & Session("Corte")  
if strdiretoria <> "0" then
	str_Sql = str_Sql & " AND ORLO_CD_GABINETE = " & strdiretoria 
else
	str_Sql = str_Sql & " AND ORLO_CD_GABINETE = 99999999 " 
end if
str_Sql = str_Sql & " AND ORLO_CD_STATUS = 'A'"

str_Sql = str_Sql & " ORDER BY ORLO_SG_ORG_LOT "
'Response.write str_Sql & "<br><br>"
'Response.END
set rdsOrgaoMaior = db_banco.execute(str_Sql)
...
<html>
	...
	<script language="javascript">
		...
		{				
			...
			if (strTipo == 'Corte')
			{				
				window.location.href = "param_orgao_maior_menor.asp?selCorte="+document.frm1.selCorte.value+"&selDiretoria=0&selUnidade=0&pNumRel="+document.frm1.pNumRel.value+"&pTituloRel="+document.frm1.pTituloRel.value;
			}			
			
			if (strTipo == 'Diretoria')
			{
				window.location.href = "param_orgao_maior_menor.asp?selCorte="+document.frm1.selCorte.value+"&selDiretoria="+document.frm1.selDiretoria.value+"&selUnidade=0&pNumRel="+document.frm1.pNumRel.value+"&pTituloRel="+document.frm1.pTituloRel.value;
			}				
		}
		...
		{				
			if (document.frm1.selCorte.selectedIndex == 0)
			{
				...
				document.form1.selCorte.focus();
				return;
			}
			
			if (document.frm1.pNumRel.value == '1')
			{
				document.frm1.action = "dados_orgao_maior_menor.asp";
			}
									
			if (document.frm1.pNumRel.value == '2')
			{
				document.frm1.action = "gera_consulta_demanda.asp";
			}
...
<body topmargin="0" leftmargin="0" bgcolor="#FFFFFF">
	<form method="POST" name="frm1">	
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#330099">
		  ...
		  <table border="0" width="849" height="154">					
			<tr>
			  <td height="26"></td>
			...
			<tr>
			  <td height="26"></td>
			  <td height="26" valign="middle" align="left"><font face="Verdana" size="2" color="#330099"><b>Orgao Maior: </b></font></td>
			  <td height="26" valign="middle" align="left">
			  	
				<select size="1" name="selOrgaoMaior">
					<option value="0">== TODAS ==</option>
						<%
						do until rdsOrgaoMaior.eof = true
							  if cint(strCdOrgaoMaior) = cint(rdsOrgaoMaior("ORLO_CD_ORG_LOT")) then%>
								<option value="<%=rdsOrgaoMaior("ORLO_CD_ORG_LOT")%>" selected><%=rdsOrgaoMaior("ORLO_SG_ORG_LOT")%></option>
							<%else%>
								<option value="<%=rdsOrgaoMaior("ORLO_CD_ORG_LOT")%>"><%=rdsOrgaoMaior("ORLO_SG_ORG_LOT")%></option>
							<%end if						
							rdsOrgaoMaior.movenext
						loop
						
						rdsOrgaoMaior.close
						set rdsOrgaoMaior = nothing
						%>
           	  </select>				 
		    </tr>		
