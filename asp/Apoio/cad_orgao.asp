...
if orgao_2 <> "M9999" then
	...
	ssql3=""
	ssql3="SELECT  ORLO_CD_ORG_LOT, ORME_CD_ORG_MENOR, AGLU_CD_AGLUTINADO, ORME_SG_ORG_MENOR, ORME_CD_STATUS, ORME_NM_ORG_MENOR FROM dbo.ORGAO_MENOR "
	ssql3=ssql3+" WHERE (AGLU_CD_AGLUTINADO = " & ORGAO_1 & ") AND (ORME_CD_STATUS = 'A') ORDER BY ORME_SG_ORG_MENOR"
end if
...
{
carrega_txt1(document.frm1.list1);
document.frm1.action="valida_cad_orgao.asp"
document.frm1.submit();
}
}
...
<form name="frm1" method="POST" action="cad_orgao.asp">

  <input type="hidden" name="chave" size="13" value="<%=REQUEST("CHAVE")%>">
  <input type="hidden" name="atribb" size="13" value="<%=REQUEST("ATRIBB")%>">
  <input type="hidden" name="txtorgao" size="78" value="<%=REQUEST("LISTA")%>">
  <input type="hidden" name="opti" size="78" value="1">
...
Do Until contador = tamanho + 1
    ...
    If str_temp = "," Then
        ...
        select case quantos2
			...
			%>
				...
				set orgaos=db.execute("SELECT * FROM " & Session("prefixo") & "APOIO_LOCAL_ORGAO WHERE USMA_CD_USUARIO='" & REQUEST("CHAVE") & "' AND APLO_NR_ATRIBUICAO=" & REQUEST("ATRIBB"))
    ...
    <tr> 
      ...
      <td width="20%"> <select name="Str02" size="1" onChange="manda02()">
          ...
          <%
          if recebe=0 and orgao_1<>0 then
          if orgao_2="M9999" then
          	vis = "selected"
          else
			vis = ""          
          ...
          %>
          <option <%=vis%> value="M9999">== TODAS AS LOTAÇÕES ==</option>
          <%
          end if
        ...
        %>
        <input type="hidden" name="recebe" size="13" value="1">
        <script>
		{
		document.frm1.Str02.options[2].selected=true
		carrega_txt1(document.frm1.list1);
		document.frm1.submit();
		}
		  </script>
    ...
    <tr> 
      ...
      <td width="20%" height="28" rowspan="2"> <table width="59%" border="0" cellpadding="0" cellspacing="0">
       <tr> 
        ...
        local = InStr(1,request("txtorgao"), str3("ORME_CD_ORG_MENOR"))
      ...
      <tr> 
        <td><div align="center"><a href="#"><img src="selecionar.gif" width="58" height="40" border="0" onClick="Seleciona_tudo()"></a></div></td>
       </tr>
     </table></td>
