...
ssql=ssql+"WHERE (dbo.APOIO_LOCAL_MULT.APLO_NR_ATRIBUICAO=1) AND (dbo.APOIO_LOCAL_MULT.APLO_TX_OBS NOT LIKE '') AND (dbo.APOIO_LOCAL_MULT.APLO_TX_OBS IS NOT NULL)"
...
<table width="94%" border="0">
  <tr> 
    <td width="73%"><div align="left"><font color="#000066" size="2" face="Verdana, Arial, Helvetica, sans-serif"><strong>CONSULTA 
        DE OBSERVA&Ccedil;&Otilde;ES POR ASSUNTO - APOIADOR LOCAL</strong></font></div></td>
    <td width="27%"><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Localizar 
...
%>
  ...
  if seleciona=1 then
	...
	ssql=ssql+"WHERE (dbo.APOIO_LOCAL_MULT.APLO_TX_OBS NOT LIKE '') AND (dbo.APOIO_LOCAL_MULT.APLO_NR_SITUACAO = 1) AND (dbo.APOIO_LOCAL_MODULO.APLO_NR_ATRIBUICAO=1) AND (dbo.APOIO_LOCAL_MULT.APLO_TX_OBS IS NOT NULL) AND (dbo.APOIO_LOCAL_MODULO.SUMO_NR_CD_SEQUENCIA = " & modulo & ") "
  ...
  <tr bgcolor="#CCCCCC"> 
    ...
    <td width="66" bgcolor="#CCCCCC"><div align="left"><strong><font color="#0000CC" size="2" face="Arial, Helvetica, sans-serif">Obs</font></strong></div></td>
  ...
  <tr valign="middle" bgcolor="<%=cor%>"> 
    ...
    %>
    <td width="117" valign="top"> <%ON ERROR RESUME NEXT %> <p align="left"><font size="1" face="Arial, Helvetica, sans-serif"><%=temp("lotacao")%></font></td>
    ...
    <%
		ssql=""
		ssql="select aplo_tx_obs from apoio_local_mult where (usma_cd_usuario='" & UCASE(rs("CHAVE")) & "') "
		ssql=ssql+"AND (APLO_TX_OBS NOT LIKE '') AND (APLO_TX_OBS IS NOT NULL)"
	  
	  set obsr=db.execute(ssql)
	  
	  observ=obsr("aplo_tx_obs")
	  
	  %>
    <td width="66" valign="top"> <div align="center"> 
        <pre><font size="1" face="Arial, Helvetica, sans-serif"><%=observ%></font></pre>
      </div></td>
    <%
		ssql=""
    ...
    <%
    	ssql=""
    	ssql="SELECT DISTINCT ORME_CD_ORG_MENOR AS APOIO FROM APOIO_LOCAL_ORGAO WHERE (USMA_CD_USUARIO ='" & UCASE(rs("chave")) & "') AND (APLO_NR_ATRIBUICAO=1) AND SUBSTRING(ORME_CD_ORG_MENOR,8,8) = '00000000' ORDER BY ORME_CD_ORG_MENOR"
    ...
    %>
    <td valign="top" width="111"> <font size="1" face="Arial, Helvetica, sans-serif"> 
      <div align="left"><%=orgao%></A></div></td>
  </tr>
  <%
  tem=tem+1
  rs.movenext
