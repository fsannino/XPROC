...
if is_cli.eof=false then
	...
	texto_cli=texto_cli + "ORME_CD_ORG_MENOR LIKE '" & orgao_cli & "%' OR "
	
	cli.movenext
	loop
	
	texto_cli = left(texto_cli,len(texto_cli)-4)

	if request("selOrgao")=0 then
		set users = db.execute("SELECT DISTINCT USMA_CD_USUARIO, USMA_TX_NOME_USUARIO FROM USUARIO_MAPEAMENTO WHERE (" & texto_cli & ") AND USMA_CD_USUARIO<>'" & chave & "' ORDER BY USMA_TX_NOME_USUARIO")
	else	
...
if orgao_cli="0" then
	...
	ssql="SELECT DISTINCT ORME_CD_ORG_MENOR, ORME_SG_ORG_MENOR FROM ORGAO_MENOR WHERE (" & texto_cli & ") AND ORME_CD_STATUS='A' ORDER BY ORME_SG_ORG_MENOR"
