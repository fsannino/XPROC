...
if chave <>"XXXX" then
	ssql=ssql+" WHERE APOIO_LOCAL_CURSO.USMA_CD_USUARIO='" & chave & "' AND USUARIO_MAPEAMENTO.ORME_CD_ORG_MENOR LIKE '" & lotacao & "%' ORDER BY USUARIO_MAPEAMENTO.USMA_TX_NOME_USUARIO, APOIO_LOCAL_CURSO.CURS_CD_CURSO"
else
	ssql=ssql+" WHERE USUARIO_MAPEAMENTO.ORME_CD_ORG_MENOR LIKE '" & lotacao & "%' ORDER BY USUARIO_MAPEAMENTO.USMA_TX_NOME_USUARIO, APOIO_LOCAL_CURSO.CURS_CD_CURSO"
end if
...
<table border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="89%" id="AutoNumber2" height="514">
           ...
           <tr>
                      ...
                      <td width="87%" height="445" valign="top">
                         ...
                         <p style="margin-top: 0; margin-bottom: 0"><b><font face="Verdana" color="#800000">Consulta Multiplicador x Curso</font></b></p>
<%if rs.eof=true then%>
                         <p style="margin-top: 0; margin-bottom: 0">&nbsp;</p>
...
<%else%>
                         ...
                         <table border="1" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#D7D5CC" width="97%" id="AutoNumber4" height="43">
                                    <tr>
                                               <td width="36%" height="22" bgcolor="#666633"><b><font size="2" face="Verdana" color="#FFFFFF">Multiplicador</font></b></td>
                                               <td width="64%" height="22" bgcolor="#666633"><b><font size="2" face="Verdana" color="#FFFFFF">Curso</font></b></td>
                                    </tr>
                                    <%
                                    atual=""
                                    anterior=""
									...
									%>
                                    <tr>
                                               <td width="36%" height="20"><font size="1" face="Verdana"><b><%=rs("USMA_CD_USUARIO")%> - <%=rs("USMA_TX_NOME_USUARIO")%></b></font></td>
                                               <td width="64%" height="20"><font size="1" face="Verdana"><%=rs("CURS_TX_NOME_CURSO")%></font></td>
                                    </tr>
                                    <%
									tem = tem + 1
									end if
									err.clear
									anterior=rs("USMA_CD_USUARIO")
									rs.movenext
                                    loop
                                    if tem=0 then
									%>
