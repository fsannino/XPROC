...
str_tran=request("selTrans")

set rs=db.execute("SELECT * FROM " & Session("PREFIXO") & "TRANSACAO WHERE TRAN_CD_TRANSACAO='"& str_tran & "'")
%>
<html>
<head>
<script>
function Confirma() 
{ 
if (document.frm1.DescTransacao.value == "")
     { 
	 alert("A Descrição de uma Transação é obrigatório!");
     document.frm1.DescTransacao.focus();
     return;
     }
 }
</SCRIPT>
...
<form name="frm1" method="POST" action="altera_transacao2.asp">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr> 
      <td width="8%"></td>
      <td width="13%"><b><font face="Verdana, Arial, Helvetica, sans-serif" color="#000099" size="2">Transação</font></b></td>
      <td width="63%"><font size="2" face="Verdana, Arial, Helvetica, sans-serif"> 
        <p> 
          <input type="text" name="DescTransacao" size="75" value="<%=rs("TRAN_TX_DESC_TRANSACAO")%>">
        </p>
        </font></td>
      <td width="16%">&nbsp;</td>
    </tr>
    ...
    <tr> 
      ...
      <td width="13%"></td>
      <td width="63%">&nbsp; 
        <input type="hidden" name="CodTransacao" size="7" value="<%=rs("TRAN_CD_TRANSACAO")%>">
      </td>
      <td width="16%">&nbsp;</td>
    </tr>
    <tr> 
