...
function Confirma2() 
{ 
  window.location.href='relacao_usu_mega_proc_.asp'
 }
...
function MM_goToURL1() { //v3.0
 ...
 // for (i=0; i<(args.length-1); i+=2) eval(args[i]+".location='"+args[i+1]+"?selUsuario="+document.frm1.selUsuario.value+"'");
}
function Confirma() 
{ 
...
<form name="frm1" method="post" action="grava_acesso.asp">
  ...
  <table width="90%" border="0" cellpadding="0" cellspacing="0">
    <tr>
      <td width="6%">&nbsp;</td>
      <td width="28%">aa<%=str_Usuario%></td>
      <td width="66%">&nbsp;</td>
    </tr>
    <tr> 
      <td width="6%">&nbsp;</td>
      <td width="28%">bb<%=request("selUsuario")%></td>
      <td width="66%"><font color="#003366" size="3" face="Verdana, Arial, Helvetica, sans-serif">Acesso 
    ...
    <tr> 
      <td width="6%">&nbsp;</td>
      <td width="28%"><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#003366"><b>Usu&aacute;rio</b></font></td>
      <td width="66%"> 
        <select name="selUsuario" size="1" onChange="MM_goToURL1('self','relacao_usu_mega_proc.asp');return document.MM_returnValue">
          <option value="0" selected>Selecione um Usuário</option>
          <%DO UNTIL RS.EOF=TRUE
          if trim(str_Usuario)=trim(RS("USUA_CD_USUARIO")) then
          %>
              <option selected value=<%=RS("USUA_CD_USUARIO")%>><%=RS("USUA_TX_NOME_USUARIO")%></option>
          <%else%>
              <option value=<%=RS("USUA_CD_USUARIO")%>><%=RS("USUA_TX_NOME_USUARIO")%></option>
			<%
			end if
			RS.MOVENEXT
			LOOP
    ...
    <tr> 
      <td width="6%">&nbsp;</td>
      <td width="28%" valign="top"><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#003366"><b>Selecione 
        Mega-Processo </b></font></td>
      <td width="66%"> 
