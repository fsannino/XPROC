...
If str_OPT = 1 then
	str_Titulo = "Seleção de funções para liberação para mapeamento"
else
	str_Titulo = "Seleção de funções para classificação"
end if
str_SQL_MegaProc = ""
...
function manda()
{	
	window.location.href='sel_func_libera_mapeamento.asp?selMegaProcesso='+document.frm1.selMegaProcesso.value+'&pOpt='+document.frm1.txtOPT.value
}
...
function Confirma()
{
	 if(document.frm1.txtOPT.value == 1)
	   {
	   document.frm1.action="relaciona_func_libera_mapeamento.asp";
	 ...
	 if(document.frm1.txtOPT.value == 2)
	   {
			...
			{
			 	document.frm1.action="relaciona_func_classifica.asp";			   
			    document.frm1.submit();
			}
	   }
}
