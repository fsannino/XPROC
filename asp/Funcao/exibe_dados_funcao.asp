...
'*** Variável que define se a função herdou as transações de outra função.
str_funcao_pai = rs_func("FUNE_CD_FUNCAO_NEGOCIO_PAI")
...
if len(trim(compl))>0 then
	if str_funcao_pai <> "" and str_funcao_pai <> null then
		compl=" WHERE " & LEFT(compl,tamanho) & " AND FUNE_CD_FUNCAO_NEGOCIO='"& str_funcao_pai &"'"
	else
   		compl=" WHERE " & LEFT(compl,tamanho) & " AND FUNE_CD_FUNCAO_NEGOCIO='"& str_funcao &"'"
	end if
else
	if str_funcao_pai <> "" and str_funcao_pai <> null then
		compl=" WHERE FUNE_CD_FUNCAO_NEGOCIO='"& str_funcao_pai &"'"
	else
   		compl=" WHERE FUNE_CD_FUNCAO_NEGOCIO='"& str_funcao &"'"
	end if	
end if
...
<%=rs_func("FUNE_TX_TITULO_FUNCAO_NEGOCIO")%></font></td>
      ...
      <td width="205" height="25" valign="top"> <font face="Verdana" size="1" color="#330099"><b>&Aacute;rea 
        de Abrangência</b></font></td>
            <%
...
<div align="center">
  <center>
    <table border="0" width="73%" cellspacing="1" cellpadding="0">
      <%
  ...
  <center>
      ...
      %>
        <p style="margin-top: 0; margin-bottom: 0"><font face="Verdana" size="1" color="#330099">- <%=VALOR_TRANS%></font></td>
    </tr>
    <%
    RS_TRANS.MOVENEXT
    LOOP
