...
set rs=conn_db.execute("SELECT * FROM " & Session("PREFIXO") & "MACRO_PERFIL WHERE MEPR_CD_MEGA_PROCESSO=" & request("MEGA")  & " AND MCPE_TX_SITUACAO='EA' OR MCPE_TX_SITUACAO='EC' OR MCPE_TX_SITUACAO='NA'")
%>
<html>
<head>
<STYLE type=text/css>
    ...
    <td colspan="3" height="20">
      &nbsp;
        ...
        <table width="512" border="0" cellpadding="0" cellspacing="0" align="center">
          <tr>
            <td height="41" width="176" align="right"><a href="selec_valida_status2.asp"><img src="file:///G:/xproc/imagens/selecao_F02.gif" width="22" height="20" border="0"></a></td>
        ...
        <%do until rs.eof=true
          	...
          	'RESPONSE.WRITE REQUEST("macro_" & trim(rs("MCPE_TX_NOME_TECNICO")))
        	...
        	SET ATUAL=CONN_DB.EXECUTE("SELECT * FROM " & Session("PREFIXO") & "MACRO_PERFIL WHERE MCPE_TX_NOME_TECNICO='" & rs("MCPE_TX_NOME_TECNICO") & "'")
      		VALOR_ATUAL=ATUAL("MCPE_TX_SITUACAO")
        		
      		IF TRIM(VALOR_ATUAL)<>TRIM(VALOR_STATUS) THEN
        	
				CONN_DB.EXECUTE("UPDATE " & Session("PREFIXO") & "MACRO_PERFIL SET MCPE_TX_SITUACAO='" &  VALOR_STATUS & "' WHERE MCPE_TX_NOME_TECNICO='" & rs("MCPE_TX_NOME_TECNICO") & "'")
        		...
        		SSQL=SSQL+"VALUES(" & ATUAL &", " & rs("MCPR_NR_SEQ_MACRO_PERFIL") & ", '"& VALOR_STATUS &"', 'I', '" & session("cdUsuario") & "', GETDATE())"
