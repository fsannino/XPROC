...
set rs=conn_db.execute("SELECT * FROM " & Session("PREFIXO") & "MACRO_PERFIL WHERE MEPR_CD_MEGA_PROCESSO=" & request("selMegaProcesso")  & " AND (MCPE_TX_SITUACAO = 'EE' OR MCPE_TX_SITUACAO = 'AT' OR  MCPE_TX_SITUACAO = 'EA' OR  MCPE_TX_SITUACAO = 'RD' OR  MCPE_TX_SITUACAO = 'RE' OR  MCPE_TX_SITUACAO = 'EC') ORDER BY MCPE_TX_NOME_TECNICO")
%>
<html>
<head>
<STYLE type=text/css>
...
<form name="frm1" method="post" action="valida_status1.asp">
  ...
  <p align="center"><font color="#330099" face="Verdana" size="3">Encaminhamento 
    de Status :&nbsp; Em Elaboração -&gt; Em Aprovação/Em Cria&ccedil;&atilde;o 
    R/3 
    <input type="hidden" name="usuario" size="10" value="<%=Session("CdUsuario")%>"></font></p>
  ...
  <table width="83%" border="0" cellpadding="0" cellspacing="4">
    <tr>
      ...
      <td width="27%"><div align="right"><font face="Verdana" size="1"><img border="0" src="../../imagens/icon_empresa.gif" alt="Visualizar Histórico">Mostra 
          o hist&oacute;rico passo a passo</font></div></td>
    </tr>
  </table>
  <table border="0" width="949">
    ...
    <tr> 
      ...
      <td width="130" align="center" valign="middle"> <font color="#330099" face="Verdana" size="1"><a href="exibe_transacao_macro.asp?selMegaProcesso=<%=request("selMegaProcesso")%>&txtOPT=3&selMacroPerfil=<%=RS("MCPR_NR_SEQ_MACRO_PERFIL")%>"><b><%=trim(RS("MCPE_TX_NOME_TECNICO"))%></b></a></font> 
      ...
      <%IF VALOR2="X" THEN%>
      <td width="18" align="center" valign="middle" bgcolor="#FFFFFF"> <p align="left"><a href="rel_funcao_transacao.asp?selmacroperfil=<%=trim(RS("MCPR_NR_SEQ_MACRO_PERFIL"))%>&amp;txtopt=3"></a></p></td>
      <td width="34" align="center" valign="middle" bgcolor="#FFFFFF"> <div align="left"><a href="#" onclick="ver_historico('<%=trim(RS("MCPR_NR_SEQ_MACRO_PERFIL"))%>')"></a> 
        </div></td>
      <%ELSE%>
