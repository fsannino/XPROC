...
if str_Acao = "C" then 
   ...
   if str_MegaProcesso = 15 then
	   ssql=ssql+ " , MCPE_TX_NOME_DERIVACAO"
	   ssql=ssql+ " , MCPE_BO_DERIVACAO"
   end if
   ...
   if str_MegaProcesso = 15 then
	if request("chkDeriva") = 1 then 	   
		ssql=ssql+ ",0"
		ssql=ssql+ "," & request("chkDeriva")
	else
		ssql=ssql+ "," & request("selDeriva")
		ssql=ssql+ ",0"
	end if
   end if
	  ...
	  loo_Existe = False
		...
		IF not rdsExiste2.EOF then
			Do While not rdsExiste2.EOF
				'response.write "Dono..." & rs_QtdMega("TRAN_CD_TRANSACAO") & " - " & rdsExiste2("MEPR_CD_MEGA_PROCESSO") & "<br>"
...
'**********************************************************************************************
   if str_MegaProcesso = 15 and request("chkDeriva") <> 1 then
  ...
  ' response.redirect "rel_funcao_transacao.asp?selMegaProcesso=" & str_MegaProcesso & "&selFuncao=" & str_FuncPrinc & "&txtMacroPerfil=" & sequencia & "&txtNomeTecnico=" & str_PrefixoNomeTecnico & str_NomeTecnico & "&txtOPT=1"
   ...
   If Len(str_Especificacao) <> 0 OR Trim(str_DescDetaMacroPerfil_Original) <> Trim(str_DescDetaMacroPerfil) OR Trim(str_DescMacroPerfil_Original) <> Trim(str_DescMacroPerfil) OR Trim(str_NomeTecnico_Original) <> Trim(str_NomeTecnico) then 
	  ...
	  if str_MegaProcesso = 15 then
		if request("chkDeriva") = 1 then 	   
	      ssql=ssql+ " , MCPE_TX_NOME_DERIVACAO = 0 "   
    	  ssql=ssql+ " , MCPE_BO_DERIVACAO = " & request("chkDeriva")
		else
	      ssql=ssql+ " , MCPE_TX_NOME_DERIVACAO = " &  request("selDeriva")
    	  ssql=ssql+ " , MCPE_BO_DERIVACAO = 0 "
    	  
    	  db.execute("UPDATE MACRO_PERFIL SET MCPE_TX_NOME_DERIVACAO=0 WHERE MCPE_TX_NOME_DERIVACAO = " & str_MacroPerfil)
