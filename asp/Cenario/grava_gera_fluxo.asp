...
do while int_contador <= (int_QtdObj+1)-1
	...
	call Grava_dados(str_Cenario,str_GeraFluxo,str_GeraFluxo_Ant)
...
Sub Grava_dados(p_Cenario,p_GeraFluxo,p_GeraFluxo_Ant)
	if p_GeraFluxo = "S" then
		ssql=" UPDATE " & Session("PREFIXO") & "CENARIO SET CENA_TX_GERA_FLUXO = 'S' "
		ssql= ssql & ", CENA_TX_CD_CENARIO = NULL "
		p_GeraFluxo = "S"
	else
		ssql=" UPDATE " & Session("PREFIXO") & "CENARIO SET CENA_TX_GERA_FLUXO = 'N' "
		p_GeraFluxo = "N"	
	end if
	...
	if p_GeraFluxo_Ant = "S" then
	   if p_GeraFluxo = "N" then
	      str_SQL = ""
		  str_SQL = str_SQL & " UPDATE " & Session("PREFIXO") & "CENARIO SET CENA_TX_CD_CENARIO = NULL "
          ...
          str_SQL = str_SQL & " WHERE CENA_TX_CD_CENARIO = '" & p_Cenario & "'"
	   end if
'	else
'	   if p_GeraFluxo = "N" then
'	      str_SQL = ""
...
'	      str_SQL = str_SQL & " ,ATUA_TX_OPERACAO = 'A'"   
 '         str_SQL = str_SQL & " ,ATUA_CD_NR_USUARIO = '" & Session("CdUsuario") & "'"
  '        str_SQL = str_SQL & " ,ATUA_DT_ATUALIZACAO = GETDATE()"
   '       str_SQL = str_SQL & " WHERE CENA_CD_CENARIO = '" & p_Cenario & "'"	   
	'   end if   
	end if
   	conn_db.execute(ssql)
end sub
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr> 
      ...
      <td width="62%"> <font size="3" face="Verdana, Arial, Helvetica, sans-serif" color="#000099">Grava 
        altera&ccedil;&atilde;o de gera fluxo</font></td>
      <td width="14%">&nbsp;</td>
    </tr>
  </table>
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
