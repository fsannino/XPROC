...
do while int_contador <= (int_QtdObj+1)-1
	...
	str_Sequencia =  request("txtSeq" & int_contador)
	call f_Altera_Sequencia(str_Cenario,str_Sequencia)
	int_contador = int_contador + 1
'    response.write int_contador
loop

Sub f_Altera_Sequencia(p_Cenario, p_Sequencia)
	if p_Sequencia <> "" then
		ssql=" UPDATE " & Session("PREFIXO") & "CENARIO SET CENA_NR_SEQUENCIA_ORDEM = " & p_Sequencia
	else
		ssql=" UPDATE " & Session("PREFIXO") & "CENARIO SET CENA_NR_SEQUENCIA_ORDEM = Null "	
	end if
