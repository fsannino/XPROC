...
Dim str_SubProcesso
Dim str_Cenario
Dim str_CenarioSequencia

str_MegaProcesso = "0"
str_Processo = "0"
str_SubProcesso = "0"
str_Cenario = 0
str_CenarioSequencia = 0
...
if (Request("p_Cen_Sequencia") <> "") then 
    str_CenarioSequencia = Request("p_Cen_Sequencia")
else
    str_CenarioSequencia = "0"
end if
...
str_SQL_Cen_Tran = str_SQL_Cen_Tran & " AND " & Session("PREFIXO") & "CENARIO_TRANSACAO.CETR_NR_SEQUENCIA = " &  str_CenarioSequencia
...
str_SQL_DescCenario = str_SQL_Cenario
...
str_SQL_BPP = str_SQL_BPP & " order by BPPP_TX_DESC_BPP"
...
<form name="frm1" method="POST" action="grava_operacoes_especiais.asp">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr> 
      ...
      <td width="62%"><font size="3" face="Verdana, Arial, Helvetica, sans-serif" color="#000099">Altera 
        opera&ccedil;&atilde;o especial para um Cen&aacute;rio</font></td>
      <td width="14%">&nbsp;</td>
    </tr>
  </table>
  ...
  <table width="779" border="0" cellspacing="4" cellpadding="0">
    <tr> 
      ...
      <td width="163">&nbsp;</td>
      <td width="586"> 
        <input type="hidden" name="txtCenSeq" value="<%=str_CenarioSequencia%>">
      </td>
    </tr>
    <tr> 
      <td width="14">&nbsp;</td>
      ...
      <td width="586"> 
        <input type="text" name="txtSeq" size="4" maxlength="4" value="<%=str_Seq_Tran%>">
      </td>
    </tr>
    <tr> 
      <td width="14">&nbsp;</td>
      ...
      <td width="586"> 
        <input type="text" name="txtDescOprEs" size="60" maxlength="150" value="<%=str_Desc_Transacao%>">
      </td>
    </tr>
    <tr>
      <td width="14">&nbsp;</td>
...
While (NOT rdsOpEs.EOF)
          if (Trim(str_OpeEsp) = Trim(rdsOpEs.Fields.Item("OPES_CD_OPERACAO_ESP").Value)) then %>
...
While (NOT rdsCenario.EOF)
          ...
          <% else %>
          <option value="<%=(rdsCenario.Fields.Item("CENA_CD_CENARIO").Value)%>"><%=(rdsCenario.Fields.Item("CENA_TX_TITULO_CENARIO").Value)%></option>
          <% end if %>
          <%
  rdsCenario.MoveNext()
Wend
...
%>
    ...
    <tr> 
      ...
      <td width="586"> 
        <select name="selBPP">
          <option value="0" selected>Sem BPP</option>
          <%Set rdsBPP= Conn_db.Execute(str_SQL_BPP)
While (NOT rdsBPP.EOF)
          if (Trim(str_BPP) = Trim(rdsBPP.Fields.Item("BPPP_CD_BPP").Value)) then %>
          <option value="<%=(rdsBPP.Fields.Item("BPPP_CD_BPP").Value)%>" selected ><%=(rdsBPP.Fields.Item("BPPP_TX_DESC_BPP").Value)%></option>
          <% else %>
          <option value="<%=(rdsBPP.Fields.Item("BPPP_CD_BPP").Value)%>"><%=(rdsBPP.Fields.Item("BPPP_TX_DESC_BPP").Value)%></option>
          <% end if %>
          <%
  rdsBPP.MoveNext()
Wend
If (rdsBPP.CursorType > 0) Then
  rdsBPP.MoveFirst
Else
  rdsBPP.Requery
End If
...
%>
    ...
    <tr> 
      <td width="14">&nbsp;</td>
      <td width="163">&nbsp;</td>
      <td width="586"><font face="Verdana, Arial, Helvetica, sans-serif" size="2"> 
        </font></td>
    </tr>
  </table>
  <p>&nbsp; </p>
