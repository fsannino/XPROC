...
if (Request("selOpEs") <> "") then 
    str_OpEs = Request("selOpEs")
else
    str_OpEs = "0"
end if

if (Request("selDesenv") <> "") then 
...
str_SQL_Cen_Tran = str_SQL_Cen_Tran & " , " & Session("PREFIXO") & "CENARIO_TRANSACAO.DESE_CD_DESENVOLVIMENTO "
...
if not rdsCen_Tran.EOF then
   ...
   str_OpEs = rdsCen_Tran("OPES_CD_OPERACAO_ESP")
   str_Desenv = rdsCen_Tran("DESE_CD_DESENVOLVIMENTO")      
else
   str_CenarioTrSequencia = "0"
   str_Desc_Transacao = "não achou"
   str_OpEs = "0"
   str_Desenv = "0"
end if
rdsCen_Tran.Close
...
str_SQL_OpEs = str_SQL_OpEs & " where OPES_CD_OPERACAO_ESP IN (1 , 2) "
str_SQL_OpEs = str_SQL_OpEs & " order by OPES_TX_DESC_OPERACAO_ESP "

str_SQL_Desenv = ""
str_SQL_Desenv = str_SQL_Desenv & " SELECT "
str_SQL_Desenv = str_SQL_Desenv & " DESE_CD_DESENVOLVIMENTO, "
str_SQL_Desenv = str_SQL_Desenv & " DESE_TX_DESC_DESENVOLVIMENTO"
str_SQL_Desenv = str_SQL_Desenv & " FROM " & Session("PREFIXO") & "DESENVOLVIMENTO"
str_SQL_Desenv = str_SQL_Desenv & " order by DESE_CD_DESENVOLVIMENTO "
...
if (document.frm1.selOpEs.selectedIndex == 0)
     { 
	 alert("A seleção de um Tipo de Operação Especial é obrigatório!");
     document.frm1.selOpEs.focus();
     return;
     }
if (document.frm1.selDesenv.selectedIndex == 0)
     { 
	 alert("A seleção de um Desenvolvimento é obrigatório!");
     document.frm1.selDesenv.focus();
     return;
     }	 	 
...
<form name="frm1" method="POST" action="grava_operacoes_especiais.asp">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr> 
      <td width="24%">&nbsp;</td>
      <td width="62%"><font size="3" face="Verdana, Arial, Helvetica, sans-serif" color="#000099">Altera</font><font size="3" face="Verdana, Arial, Helvetica, sans-serif" color="#000099"> 
        opera&ccedil;&atilde;o exit/interface para um Cen&aacute;rio</font></td>
      <td width="14%">&nbsp;</td>
    </tr>
  </table>
  ...
  <table width="779" border="0" cellspacing="2" cellpadding="0">
    <tr> 
      <td width="125"> 
        <input type="hidden" name="txtAcao" value="AED">
      </td>
      <td width="648"> 
    ...
    <tr> 
      <td width="125"> 
        <div align="right"><b><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000099">Tipo 
          Oper Esp </font></b></div>
      </td>
      <td width="648"> 
        <select name="selOpEs">
          <option value="0" selected>Sem Oper. Especial</option>
          <%Set rdsOpEs= Conn_db.Execute(str_SQL_OpEs)
While (NOT rdsOpEs.EOF)
         if (Trim(str_OpEs) = Trim(rdsOpEs.Fields.Item("OPES_CD_OPERACAO_ESP").Value)) then %>
          <option value="<%=(rdsOpEs.Fields.Item("OPES_CD_OPERACAO_ESP").Value)%>" selected ><%=(rdsOpEs.Fields.Item("OPES_TX_DESC_OPERACAO_ESP").Value)%></option>
          <% else %>
          <option value="<%=(rdsOpEs.Fields.Item("OPES_CD_OPERACAO_ESP").Value)%>" ><%=(rdsOpEs.Fields.Item("OPES_TX_DESC_OPERACAO_ESP").Value)%></option>
          <% end if %>
          <%
  rdsOpEs.MoveNext()
Wend
If (rdsOpEs.CursorType > 0) Then
  rdsOpEs.MoveFirst
Else
  rdsOpEs.Requery
End If
    ...
    <tr> 
      <td width="125"> 
        <div align="right"><b><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000099"> 
          Desenvolvimento </font></b></div>
      </td>
      <td width="648"> 
        <select name="selDesenv">
          <option value="0" selected>Selecione um Desenvolvimento</option>
          <%Set rdsDesenv= Conn_db.Execute(str_SQL_Desenv)
While (NOT rdsDesenv.EOF)
         if (Trim(str_Desenv) = Trim(rdsDesenv.Fields.Item("DESE_CD_DESENVOLVIMENTO").Value)) then %>
          <option value="<%=(rdsDesenv.Fields.Item("DESE_CD_DESENVOLVIMENTO").Value)%>" selected><%=rdsDesenv.Fields.Item("DESE_CD_DESENVOLVIMENTO").Value%> - <%=Left((rdsDesenv.Fields.Item("DESE_TX_DESC_DESENVOLVIMENTO").Value),70)%></option>
          <% else %>
          <option value="<%=(rdsDesenv.Fields.Item("DESE_CD_DESENVOLVIMENTO").Value)%>"><%=rdsDesenv.Fields.Item("DESE_CD_DESENVOLVIMENTO").Value%> - <%=Left((rdsDesenv.Fields.Item("DESE_TX_DESC_DESENVOLVIMENTO").Value),70)%></option>
          <% end if %>
          <%
  rdsDesenv.MoveNext()
Wend
If (rdsDesenv.CursorType > 0) Then
  rdsDesenv.MoveFirst
Else
  rdsDesenv.Requery
End If
    ...
    <tr> 
      <td width="125"> 
        <div align="right"><b><font size="2" color="#000099"><font face="Verdana, Arial, Helvetica, sans-serif">Desc</font><font face="Verdana, Arial, Helvetica, sans-serif"> 
          Opera&ccedil;&atilde;o especial </font></font><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000099"> 
          </font></b></div>
      </td>
      <td width="648"> 
        <input type="text" name="txtDescTransacao" size="60" maxlength="150" value="<%=str_Desc_Transacao%>">
      </td>
    </tr>
    <tr> 
      <td width="125">&nbsp;</td>
