...
IF NOT RS.EOF THEN
if not IsNull(rs("CENA_TX_SITU_DESENHO_TIPO")) then
   valor_tipo=rs("CENA_TX_SITU_DESENHO_TIPO")
else
   valor_tipo = 0
end if
if not IsNull(rs("CENA_TX_SITU_DESENHO_DESE")) then
   valor_desenv=rs("CENA_TX_SITU_DESENHO_DESE")
else
   valor_desenv = 0
end if
if not IsNull(rs("CENA_TX_SITU_DESENHO_CONF")) then
   valor_conf=rs("CENA_TX_SITU_DESENHO_CONF")
else
   valor_conf = 0
end if
if not IsNull(rs("CENA_TX_SITU_DESENHO_TESTE")) then
...
valor_teste = 0
END IF
'RESPONSE.Write(valor_tipo)
'RESPONSE.Write(valor_desenv)
'RESPONSE.Write(valor_conf)
'RESPONSE.Write(valor_teste)
%>
<html>
<head>
<STYLE type=text/css>
