...
do while int_contador <= (int_QtdObj+1)-1
	str_Cenario = request("txtCenario" & int_contador) 
	str_Assunto =  request("selAssunto" & int_contador)
	call f_Altera_Assunto(str_Cenario,str_Assunto)
	int_contador = int_contador + 1
'    response.write int_contador
loop

Sub f_Altera_Assunto(p_Cenario, p_Assunto)
	if p_Assunto <> "0" then
	   int_Qtd_Alterado = int_Qtd_Alterado + 1
	   ssql=" UPDATE " & Session("PREFIXO") & "CENARIO SET SUMO_NR_CD_SEQUENCIA = " & p_Assunto
	else
	   int_Qtd_Nulo = int_Qtd_Nulo + 1
	   ssql=" UPDATE " & Session("PREFIXO") & "CENARIO SET SUMO_NR_CD_SEQUENCIA = Null "	
	end if
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr> 
      ...
      <td width="59%"><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Quantidade 
        sem assunto = <%=int_Qtd_Nulo%></font></td>
      <td width="14%">&nbsp;</td>
    </tr>
    <tr> 
      <td width="3%">&nbsp;</td>
